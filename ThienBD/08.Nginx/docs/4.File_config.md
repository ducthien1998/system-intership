# TÌM HIỂU VỀ FILE CẤU HÌNH CỦA DỊCH VỤ NGINX

# 1. Tổng quan file cấu hình 

- Nội dung default của file 
```
user www-data;
...
#}
```
Ta chú ý đến cách viết các blocks mở đầu bằng dấu `{` và kết thúc bằng dấu `}`, nội dung bên trong 1 block hay context gồm một hoặc nhiều các `directive`. Các tùy chọn cấu hình trong Nginx được gọi là `directive`.

Các `block` có thể được viết lồng nhau giúp các `block` con kế thừa các `directive` từ `block` cha - `block` bao quanh nó. Ngoài ra, trong `block` con ta cũng có thể được ghi đè các giá trị `directive` được kế thừa từ cha bởi các giá trị theo ý muốn.
...
- Tệp bắt đầu với 4 directive: user, worker_processes, error_log, pid.
- Những `directive` này nằm ngoài cặp ngoặc nhọn và không thuộc context nào, ảnh hưởng đến toàn bộ ứng dụng ở mức cơ bản. Ta gọi nó là `main context`.
- Hai block là `events` và `http` là các `directive` bổ sung. Chúng tồn tại trong main context.

# 2. Events context

Events context nằm trong main context. Nó được sử dụng để đặt các diractive mức global, ảnh hưởng đến cách Nginx xử lý các kết nối. Chỉ có duy nhất 1 events context được định nghĩa trong file config
```
# main context
...
# 3. Http context
Đồng bậc với `events context` là `Http context`. Nó cũng nằm trong main context.   
Nếu cấu hình Nginx với mục đích làm máy chủ web hoặc reverse proxy thì http context sẽ giữ phần lớn cấu hình. Nó sẽ chứa tất cả các directive và các context cần thiết để xác định cách chương trình sẽ xử lý các kết nối HTTP hoặc HTTPS.
...
[1] [https://viblo.asia/p/tim-hieu-ve-nginx-phan-4-RQqKLo8m57z](https://viblo.asia/p/tim-hieu-ve-nginx-phan-4-RQqKLo8m57z)   
[2] [https://viblo.asia/p/tim-hieu-ve-nginx-phan-3-3P0lPGrGZox](https://viblo.asia/p/tim-hieu-ve-nginx-phan-3-3P0lPGrGZox)
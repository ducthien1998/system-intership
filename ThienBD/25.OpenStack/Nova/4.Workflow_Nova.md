...
Ở ví dụ này có 2 hosts được sử dụng: compute host (hoạt động như Hypervisor khi nova-compute chạy) và controller host(chứa tất cả các dịch vụ quản lý)
...
4. Nova kiểm tra trong database để xem có conflicts nào với tên những object đã có sẵn không và tạo mới một entry cho máy ảo mới trong database
5. Nova-api gửi RPC tới nova-scheduler service để lên lịch cho máy ảo 
6. Nova-scheduler lấy request từ message queue 
7. Nova-scheduler service sẽ tìm compute host thích hợp trong database thông qua filters và weight. Lúc này database sẽ cập nhật lại entry của máy ảo với host ID phù hợp nhận được từ nova-scheduler. Sau đó scheduler sẽ gửi RPC call tới nova-compute để khởi tạo máy ảo 
...
3. Tạo file disk: Lấy image từ glance vào thư mục instance_dir/_ base và convert nó sang dạng RAW (nếu đã ở dạng raw thì không cần convert) -> Tạo instance_dir/uuid/{disk, disk.local, disk.swap}: -> Tạo QCOW2 disk file, với backing file từ _base image -> Tạo QCOW2 "disk.local" và disk.swap
4. Tạo file libvirt XML và sap chép nó vào instance_dir (dù file này không được sử dụng bởi Nova)
...
6. Tạo network cho máy ảo 
    - Tùy thuộc vào các driver mà người dùng khai báo, các câu lệnh sẽ được thựuc hiện theo 
    - Bật tất cả các bridges/VLANs cần thiết 
    - Tạo security group (iptables)
7. Dfile domain với Libvirt, sử dụng file XML đã được tạo từ trước. Tương đương với câu lệnh virsh define instance_dir/<uuid>/libvirt.xml
8. Giờ đây máu ảo đã được start. Tương đương với câu lệnh virsh start
